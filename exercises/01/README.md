...
Let's run some terraform commands!
  ...
  - `terraform plan -out lab1.tfplan`
    ...
    * Refreshes the state of the managed resources and plans what resources will be created, updated, and removed.
    * It is a best practice to save it to a file so we apply from that plan
    * If you did not provide a default value for the variables nor provide it in the command line, it will prompt for the variable value. We will later learn how to do that

If "terraform plan" successful, you're ready to move on. For now, don't run apply. We'll get to this in a future exercise.
